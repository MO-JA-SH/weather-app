...
interface Props {
  daily: DailyForecast[];
  showModels?: boolean; // إضافة الخاصية الاختيارية
}

const Forecast: React.FC<Props> = ({ daily, showModels = false }) => {
  ...
  return (
    <>
          ...
          {daily.map((day, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-gray-600 mb-2 truncate" title={desc}>
                  {desc}
                </div>
                ...
                <div className="text-xs text-blue-600 mb-1">{day.precipitation_sum.toFixed(1)} mm</div>

                {/* عرض النماذج فقط إذا كانت showModels = true */}
                {showModels && day.modelPrecipitation && (
      ...
      <HourlyForecastModal isOpen={isModalOpen} onClose={handleCloseModal} day={selectedDay} />
    </>
  );
};
